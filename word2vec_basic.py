...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  no_of_window = batch_size // num_skips
  #using deque collection for faster appending and retrieval of data from either ends
  temp_deque = collections.deque(maxlen=window_size)
  for x in range(window_size):
      temp_deque.append(data[data_index])
      data_index = (data_index + 1) % len(data)

  for a in range(no_of_window):
      predict = skip_window  
      predict_array = [skip_window]

      for b in range(num_skips):
          while predict in predict_array:
              range_window = window_size - 1
              predict = random.randint(0, range_window)

          predict_array.append(predict)

          batch[a * num_skips + b] = temp_deque[skip_window]  
          labels[a * num_skips + b, 0] = temp_deque[predict] 
